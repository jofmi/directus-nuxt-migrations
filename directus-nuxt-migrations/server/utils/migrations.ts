...
async function getExtensionsFromDb() {
  ...
  try {
    extensions = await directus.request(readItems("schema_extensions"));
    ...
    try {
      ...
      await directus.request(
        createItem("schema_extensions", {
          extensions_name: "directus-nuxt-migrations",
...
async function runMigrations(ext: ExtensionConfig, extsDb: any[], to?: string) {
  ...
  // Update version if extension is already registered
    ...
    await directus.request(
      updateItem("schema_extensions", extensionDb.id, {
        extensions_version: ext.version,
      }),
    );
  }
